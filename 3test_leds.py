...
print("Color tests ended. Press Ctrl-C to quit")
...
#    cv2.imshow('frame', frame)
    ...
    for x in range (0,frame.shape[1],1):
        for y in range(0,frame.shape[0],1):
            ...
            #strip.setPixelColorRGB((x*y),green,red,blue)                
            #print("X1 is", x*y)
    #        time.sleep(0.0001)
            position = position +1
    for x in range(0,100,20):
       rev2 = rev2 + color[x:x+10] + color[x+10:x:-1]
       #print("List is ", len(rev2))
    # Do the Neopixel stuff
    for X1 in range(len(rev2)):
        #red = rev2[X1][1]
        #green = rev2[X1][0]
        #blue = rev2[X1][2]
        strip.setPixelColorRGB(X1,rev2[X1][0],rev2[X1][1],rev2[X1][2])
        #print("Here",X1,"Red ",red, "Green ", green, "Blue ", blue)
    strip.show() 
